...
public static class ToolCatalog
{
    ...
    public static IEnumerable<Func<ToolSpec>> CreateFactories()
    {
        ...
        yield return () => Spec("camera-recorder", ToolCategories.Media, "Camera recorder", "Capture a still image or short video from a local camera.",
            [Number("index", "Camera index", 0), Select("mode", "Mode", ["Snapshot", "Record AVI"]), Number("seconds", "Record seconds", 5), SaveFile("save", "Save video as")],
            [ImageOutput("image", "Snapshot"), Output("result", "Result")], ToolAlgorithms.CameraAsync, autoRun: false);
    ...
    private static ToolSpec Spec(
        ...
        Func<ToolRunContext, CancellationToken, Task> run,
        bool autoRun = true)
    {
