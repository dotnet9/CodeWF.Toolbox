...
using Tools.CodeWF.Bases;
using Tools.CodeWF.Fluent.Converters;
...
public class UiConfig : ConfigBase
{
	...
	internal void OnDeserialized(StreamingContext context)
	{
		if (RuntimeInformation.IsOSPlatform(OSPlatform
			    .Windows)) // On win this works perfectly. By default Wasabi will run after startup.
		{
			return;
		}
		...
		if (RuntimeInformation.IsOSPlatform(OSPlatform
			    .Linux) || // On Linux we do not start Wasabi with OS by default - because Linux users knows better.
		    RuntimeInformation
			    .IsOSPlatform(OSPlatform
				    .OSX)) // Wasabi window pops up right after startup - it is annoying UX https://github.com/zkSNACKs/WalletWasabi/pull/10190.
		{
