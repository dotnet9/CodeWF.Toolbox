...
public class Validations : ReactiveObject, IRegisterValidationMethod, IValidations
{
	...
	public bool AnyWarnings =>
		ErrorsByPropertyName.Any(x => x.Value.Any(error => error.Severity == ErrorSeverity.Warning));

	public bool AnyInfoItems =>
		ErrorsByPropertyName.Any(x => x.Value.Any(error => error.Severity == ErrorSeverity.Info));

	IEnumerable<string> IValidations.InfoItems => ErrorsByPropertyName.Values.SelectMany(x =>
		x.Where(error => error.Severity == ErrorSeverity.Info).Select(error => error.Message));

	IEnumerable<string> IValidations.Warnings => ErrorsByPropertyName.Values.SelectMany(x =>
		x.Where(error => error.Severity == ErrorSeverity.Warning).Select(error => error.Message));

	IEnumerable<string> IValidations.Errors => ErrorsByPropertyName.Values.SelectMany(x =>
		x.Where(error => error.Severity == ErrorSeverity.Error).Select(error => error.Message));
	...
	private void UpdateAndNotify(List<ErrorDescriptor> currentErrors, List<ErrorDescriptor> previousErrors,
		string propertyName)
	{
