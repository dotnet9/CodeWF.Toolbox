...
using Tools.CodeWF.Fluent.Generators.Analyzers;
...
internal class UiContextConstructorGenerator : GeneratorStep<ClassDeclarationSyntax>
{
	...
	public override void Execute(ClassDeclarationSyntax[] classDeclarations)
	{
		...
		var toGenerate =
			...
			group cls by cls.Identifier.ValueText
			into g
			select g.First();
	...
	private IEnumerable<ConstructorDeclarationSyntax> GenerateConstructors(ClassDeclarationSyntax classDeclaration,
		...
		foreach (var constructor in constructors)
		{
			...
			else
			{
				...
				var code =
					$$"""
					  {{usings}}
					  using Tools.CodeWF.Fluent.Models.UI;
